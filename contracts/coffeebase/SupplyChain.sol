    ...
    ConsumerRole
{
    ...
    // Define a modifier that checks if an item.state of a upc is Purchased
    modifier itemMustNotExistBeforeHarvesting(uint256 _upc) {
    ...
    //Define a function 'kill' if required, only the owner who has deployed the contact can kill it.
    ...
    //first check if item with _upc passed as parameter does not exist already
    //Also check if _originFarmerID has the role of farmer before he/she can harvest.
    ...
    ) public itemMustNotExistBeforeHarvesting(_upc) onlyFarmer {
